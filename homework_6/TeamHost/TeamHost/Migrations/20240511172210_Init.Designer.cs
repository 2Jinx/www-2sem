// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using TeamHost.DataAccess;

#nullable disable

namespace TeamHost.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20240511172210_Init")]
    partial class Init
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.4")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("GamePlatform", b =>
                {
                    b.Property<long>("GamesId")
                        .HasColumnType("bigint");

                    b.Property<long>("PlatformsId")
                        .HasColumnType("bigint");

                    b.HasKey("GamesId", "PlatformsId");

                    b.HasIndex("PlatformsId");

                    b.ToTable("GamePlatform");
                });

            modelBuilder.Entity("TeamHost.Entities.Category", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Categories");

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Code = "strategy",
                            Description = "Стратегии",
                            Name = "Strategy"
                        },
                        new
                        {
                            Id = 2L,
                            Code = "adventure",
                            Description = "Приключенческие игры",
                            Name = "Adventure"
                        },
                        new
                        {
                            Id = 3L,
                            Code = "horror",
                            Description = "Хоррор",
                            Name = "Horror"
                        },
                        new
                        {
                            Id = 4L,
                            Code = "simulator",
                            Description = "Симулятор",
                            Name = "Simulator"
                        },
                        new
                        {
                            Id = 5L,
                            Code = "fighting",
                            Description = "Файтинг",
                            Name = "Fighting"
                        },
                        new
                        {
                            Id = 6L,
                            Code = "survival",
                            Description = "Выживание",
                            Name = "Survival"
                        });
                });

            modelBuilder.Entity("TeamHost.Entities.Company", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("CountryId")
                        .HasColumnType("bigint");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("CountryId");

                    b.ToTable("Companies");

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            CountryId = 195L,
                            Description = "Firaxis Games is an American video game developer.",
                            Name = "Firaxis Games"
                        },
                        new
                        {
                            Id = 2L,
                            CountryId = 166L,
                            Description = "CD Projekt Red is a Polish video game developer.",
                            Name = "CD Projekt Red"
                        },
                        new
                        {
                            Id = 3L,
                            CountryId = 195L,
                            Description = "InnerSloth is an independent game development studio.",
                            Name = "InnerSloth"
                        },
                        new
                        {
                            Id = 4L,
                            CountryId = 190L,
                            Description = "Rockstar North is a British video game developer.",
                            Name = "Rockstar North"
                        },
                        new
                        {
                            Id = 5L,
                            CountryId = 245L,
                            Description = "Capcom is a Japanese video game developer and publisher.",
                            Name = "Capcom"
                        },
                        new
                        {
                            Id = 6L,
                            CountryId = 195L,
                            Description = "ConcernedApe is an independent video game developer.",
                            Name = "ConcernedApe"
                        },
                        new
                        {
                            Id = 7L,
                            CountryId = 245L,
                            Description = "FromSoftware is a Japanese video game development company.",
                            Name = "FromSoftware"
                        },
                        new
                        {
                            Id = 8L,
                            CountryId = 88L,
                            Description = "Behaviour Interactive is a Canadian video game development studio.",
                            Name = "Behaviour Interactive"
                        },
                        new
                        {
                            Id = 9L,
                            CountryId = 195L,
                            Description = "Naughty Dog is an American video game developer.",
                            Name = "Naughty Dog"
                        },
                        new
                        {
                            Id = 10L,
                            CountryId = 232L,
                            Description = "Mojang Studios is a Swedish video game developer.",
                            Name = "Mojang Studios"
                        });
                });

            modelBuilder.Entity("TeamHost.Entities.Country", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Alpha2")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Alpha3")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<long>("Code")
                        .HasColumnType("bigint");

                    b.Property<string>("Fullname")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Countries");

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Alpha2 = "AU",
                            Alpha3 = "AUS",
                            Code = 36L,
                            Fullname = "АВСТРАЛИЯ",
                            Name = "АВСТРАЛИЯ"
                        },
                        new
                        {
                            Id = 2L,
                            Alpha2 = "AT",
                            Alpha3 = "AUT",
                            Code = 40L,
                            Fullname = "АВСТРИЙСКАЯ РЕСПУБЛИКА",
                            Name = "АВСТРИЯ"
                        },
                        new
                        {
                            Id = 3L,
                            Alpha2 = "AZ",
                            Alpha3 = "AZE",
                            Code = 31L,
                            Fullname = "РЕСПУБЛИКА АЗЕРБАЙДЖАН",
                            Name = "АЗЕРБАЙДЖАН"
                        },
                        new
                        {
                            Id = 4L,
                            Alpha2 = "AL",
                            Alpha3 = "ALB",
                            Code = 8L,
                            Fullname = "РЕСПУБЛИКА АЛБАНИЯ",
                            Name = "АЛБАНИЯ"
                        },
                        new
                        {
                            Id = 5L,
                            Alpha2 = "DZ",
                            Alpha3 = "DZA",
                            Code = 12L,
                            Fullname = "АЛЖИРСКАЯ НАРОДНАЯ ДЕМОКРАТИЧЕСКАЯ РЕСПУБЛИКА",
                            Name = "АЛЖИР"
                        },
                        new
                        {
                            Id = 6L,
                            Alpha2 = "AI",
                            Alpha3 = "AIA",
                            Code = 660L,
                            Fullname = "АНГИЛЬЯ",
                            Name = "АНГИЛЬЯ"
                        },
                        new
                        {
                            Id = 7L,
                            Alpha2 = "AO",
                            Alpha3 = "AGO",
                            Code = 24L,
                            Fullname = "РЕСПУБЛИКА АНГОЛА",
                            Name = "АНГОЛА"
                        },
                        new
                        {
                            Id = 8L,
                            Alpha2 = "AD",
                            Alpha3 = "AND",
                            Code = 20L,
                            Fullname = "КНЯЖЕСТВО АНДОРРА",
                            Name = "АНДОРРА"
                        },
                        new
                        {
                            Id = 9L,
                            Alpha2 = "AQ",
                            Alpha3 = "ATA",
                            Code = 10L,
                            Fullname = "АНТАРКТИДА",
                            Name = "АНТАРКТИДА"
                        },
                        new
                        {
                            Id = 10L,
                            Alpha2 = "AG",
                            Alpha3 = "ATG",
                            Code = 28L,
                            Fullname = "АНТИГУА И БАРБУДА",
                            Name = "АНТИГУА И БАРБУДА"
                        },
                        new
                        {
                            Id = 11L,
                            Alpha2 = "AR",
                            Alpha3 = "ARG",
                            Code = 32L,
                            Fullname = "АРГЕНТИНСКАЯ РЕСПУБЛИКА",
                            Name = "АРГЕНТИНА"
                        },
                        new
                        {
                            Id = 12L,
                            Alpha2 = "AM",
                            Alpha3 = "ARM",
                            Code = 51L,
                            Fullname = "РЕСПУБЛИКА АРМЕНИЯ",
                            Name = "АРМЕНИЯ"
                        },
                        new
                        {
                            Id = 13L,
                            Alpha2 = "AW",
                            Alpha3 = "ABW",
                            Code = 533L,
                            Fullname = "ОСТРОВ АРУБА",
                            Name = "АРУБА"
                        },
                        new
                        {
                            Id = 14L,
                            Alpha2 = "AF",
                            Alpha3 = "AFG",
                            Code = 4L,
                            Fullname = "ПЕРЕХОДНОЕ ИСЛАМСКОЕ ГОСУДАРСТВО АФГАНИСТАН",
                            Name = "АФГАНИСТАН"
                        },
                        new
                        {
                            Id = 15L,
                            Alpha2 = "BS",
                            Alpha3 = "BHS",
                            Code = 44L,
                            Fullname = "СОДРУЖЕСТВО БАГАМЫ",
                            Name = "БАГАМЫ"
                        },
                        new
                        {
                            Id = 16L,
                            Alpha2 = "BD",
                            Alpha3 = "BGD",
                            Code = 50L,
                            Fullname = "НАРОДНАЯ РЕСПУБЛИКА БАНГЛАДЕШ",
                            Name = "БАНГЛАДЕШ"
                        },
                        new
                        {
                            Id = 17L,
                            Alpha2 = "BB",
                            Alpha3 = "BRB",
                            Code = 52L,
                            Fullname = "БАРБАДОС",
                            Name = "БАРБАДОС"
                        },
                        new
                        {
                            Id = 18L,
                            Alpha2 = "BH",
                            Alpha3 = "BHR",
                            Code = 48L,
                            Fullname = "КОРОЛЕВСТВО БАХРЕЙН",
                            Name = "БАХРЕЙН"
                        },
                        new
                        {
                            Id = 19L,
                            Alpha2 = "BY",
                            Alpha3 = "BLR",
                            Code = 112L,
                            Fullname = "РЕСПУБЛИКА БЕЛАРУСЬ",
                            Name = "БЕЛАРУСЬ"
                        },
                        new
                        {
                            Id = 20L,
                            Alpha2 = "BZ",
                            Alpha3 = "BLZ",
                            Code = 84L,
                            Fullname = "БЕЛИЗ",
                            Name = "БЕЛИЗ"
                        },
                        new
                        {
                            Id = 21L,
                            Alpha2 = "BE",
                            Alpha3 = "BEL",
                            Code = 56L,
                            Fullname = "КОРОЛЕВСТВО БЕЛЬГИИ",
                            Name = "БЕЛЬГИЯ"
                        },
                        new
                        {
                            Id = 22L,
                            Alpha2 = "BJ",
                            Alpha3 = "BEN",
                            Code = 204L,
                            Fullname = "РЕСПУБЛИКА БЕНИН",
                            Name = "БЕНИН"
                        },
                        new
                        {
                            Id = 23L,
                            Alpha2 = "BM",
                            Alpha3 = "BMU",
                            Code = 60L,
                            Fullname = "БЕРМУДСКИЕ ОСТРОВА",
                            Name = "БЕРМУДЫ"
                        },
                        new
                        {
                            Id = 24L,
                            Alpha2 = "BG",
                            Alpha3 = "BGR",
                            Code = 100L,
                            Fullname = "РЕСПУБЛИКА БОЛГАРИЯ",
                            Name = "БОЛГАРИЯ"
                        },
                        new
                        {
                            Id = 25L,
                            Alpha2 = "BO",
                            Alpha3 = "BOL",
                            Code = 68L,
                            Fullname = "РЕСПУБЛИКА БОЛИВИЯ",
                            Name = "БОЛИВИЯ"
                        },
                        new
                        {
                            Id = 26L,
                            Alpha2 = "BA",
                            Alpha3 = "BIH",
                            Code = 70L,
                            Fullname = "БОСНИЯ И ГЕРЦЕГОВИНА",
                            Name = "БОСНИЯ И ГЕРЦЕГОВИНА"
                        },
                        new
                        {
                            Id = 27L,
                            Alpha2 = "BW",
                            Alpha3 = "BWA",
                            Code = 72L,
                            Fullname = "РЕСПУБЛИКА БОТСВАНА",
                            Name = "БОТСВАНА"
                        },
                        new
                        {
                            Id = 28L,
                            Alpha2 = "BR",
                            Alpha3 = "BRA",
                            Code = 76L,
                            Fullname = "ФЕДЕРАТИВНАЯ РЕСПУБЛИКА БРАЗИЛИЯ",
                            Name = "БРАЗИЛИЯ"
                        },
                        new
                        {
                            Id = 29L,
                            Alpha2 = "IO",
                            Alpha3 = "IOT",
                            Code = 86L,
                            Fullname = "БРИТАНСКАЯ ТЕРРИТОРИЯ В ИНДИЙСКОМ ОКЕАНЕ (БРИТ.)",
                            Name = "БРИТАН. ТЕРРИТ."
                        },
                        new
                        {
                            Id = 30L,
                            Alpha2 = "BN",
                            Alpha3 = "BRN",
                            Code = 96L,
                            Fullname = "БРУНЕЙ-ДАРУССАЛАМ",
                            Name = "БРУНЕЙ"
                        },
                        new
                        {
                            Id = 31L,
                            Alpha2 = "BV",
                            Alpha3 = "BVT",
                            Code = 74L,
                            Fullname = "ОСТРОВ БУВЕ",
                            Name = "БУВЕ"
                        },
                        new
                        {
                            Id = 32L,
                            Alpha2 = "BF",
                            Alpha3 = "BFA",
                            Code = 854L,
                            Fullname = "БУРКИНА-ФАСО",
                            Name = "БУРКИНА-ФАСО"
                        },
                        new
                        {
                            Id = 33L,
                            Alpha2 = "BI",
                            Alpha3 = "BDI",
                            Code = 108L,
                            Fullname = "РЕСПУБЛИКА БУРУНДИ",
                            Name = "БУРУНДИ"
                        },
                        new
                        {
                            Id = 34L,
                            Alpha2 = "BT",
                            Alpha3 = "BTN",
                            Code = 64L,
                            Fullname = "КОРОЛЕВСТВО БУТАН",
                            Name = "БУТАН"
                        },
                        new
                        {
                            Id = 35L,
                            Alpha2 = "VU",
                            Alpha3 = "VUT",
                            Code = 548L,
                            Fullname = "РЕСПУБЛИКА ВАНУАТУ",
                            Name = "ВАНУАТУ"
                        },
                        new
                        {
                            Id = 36L,
                            Alpha2 = "VA",
                            Alpha3 = "VAT",
                            Code = 336L,
                            Fullname = "ПАПСКИЙ ПРЕСТОЛ (ГОСУДАРСТВО-ГОРОД ВАТИКАН)",
                            Name = "ВАТИКАН"
                        },
                        new
                        {
                            Id = 37L,
                            Alpha2 = "HU",
                            Alpha3 = "HUN",
                            Code = 348L,
                            Fullname = "ВЕНГЕРСКАЯ РЕСПУБЛИКА",
                            Name = "ВЕНГРИЯ"
                        },
                        new
                        {
                            Id = 38L,
                            Alpha2 = "VE",
                            Alpha3 = "VEN",
                            Code = 862L,
                            Fullname = "БОЛИВАРИЙСКАЯ РЕСПУБЛИКА ВЕНЕСУЭЛА",
                            Name = "ВЕНЕСУЭЛА"
                        },
                        new
                        {
                            Id = 39L,
                            Alpha2 = "VI",
                            Alpha3 = "VIR",
                            Code = 850L,
                            Fullname = "ВИРГИНСКИЕ ОСТРОВА (США)",
                            Name = "ВИРГИН. О-ВА"
                        },
                        new
                        {
                            Id = 40L,
                            Alpha2 = "VG",
                            Alpha3 = "VGB",
                            Code = 92L,
                            Fullname = "БРИТАНСКИЕ ВИРГИНСКИЕ ОСТРОВА",
                            Name = "ВИРГИН. О-ВА, БРИТАНСКИЕ"
                        },
                        new
                        {
                            Id = 41L,
                            Alpha2 = "AS",
                            Alpha3 = "ASM",
                            Code = 16L,
                            Fullname = "АМЕРИКАНСКОЕ САМОА (США)",
                            Name = "ВОСТОЧНОЕ САМОА"
                        },
                        new
                        {
                            Id = 42L,
                            Alpha2 = "VN",
                            Alpha3 = "VNM",
                            Code = 704L,
                            Fullname = "СОЦИАЛИСТИЧЕСКАЯ РЕСПУБЛИКА ВЬЕТНАМ",
                            Name = "ВЬЕТНАМ"
                        },
                        new
                        {
                            Id = 43L,
                            Alpha2 = "GA",
                            Alpha3 = "GAB",
                            Code = 266L,
                            Fullname = "ГАБОНСКАЯ РЕСПУБЛИКА",
                            Name = "ГАБОН"
                        },
                        new
                        {
                            Id = 44L,
                            Alpha2 = "HT",
                            Alpha3 = "HTI",
                            Code = 332L,
                            Fullname = "РЕСПУБЛИКА ГАИТИ",
                            Name = "ГАИТИ"
                        },
                        new
                        {
                            Id = 45L,
                            Alpha2 = "GY",
                            Alpha3 = "GUY",
                            Code = 328L,
                            Fullname = "РЕСПУБЛИКА ГАЙАНА",
                            Name = "ГАЙАНА"
                        },
                        new
                        {
                            Id = 46L,
                            Alpha2 = "GM",
                            Alpha3 = "GMB",
                            Code = 270L,
                            Fullname = "РЕСПУБЛИКА ГАМБИЯ",
                            Name = "ГАМБИЯ"
                        },
                        new
                        {
                            Id = 47L,
                            Alpha2 = "GH",
                            Alpha3 = "GHA",
                            Code = 288L,
                            Fullname = "РЕСПУБЛИКА ГАНА",
                            Name = "ГАНА"
                        },
                        new
                        {
                            Id = 48L,
                            Alpha2 = "GP",
                            Alpha3 = "GLP",
                            Code = 312L,
                            Fullname = "ГВАДЕЛУПА (ФР.)",
                            Name = "ГВАДЕЛУПА"
                        },
                        new
                        {
                            Id = 49L,
                            Alpha2 = "GT",
                            Alpha3 = "GTM",
                            Code = 320L,
                            Fullname = "РЕСПУБЛИКА ГВАТЕМАЛА",
                            Name = "ГВАТЕМАЛА"
                        },
                        new
                        {
                            Id = 50L,
                            Alpha2 = "GF",
                            Alpha3 = "GUF",
                            Code = 254L,
                            Fullname = "ФРАНЦУЗСКАЯ ГВИАНА (ФР.)",
                            Name = "ГВИАНА"
                        },
                        new
                        {
                            Id = 51L,
                            Alpha2 = "GN",
                            Alpha3 = "GIN",
                            Code = 324L,
                            Fullname = "ГВИНЕЙСКАЯ РЕСПУБЛИКА",
                            Name = "ГВИНЕЯ"
                        },
                        new
                        {
                            Id = 52L,
                            Alpha2 = "GW",
                            Alpha3 = "GNB",
                            Code = 624L,
                            Fullname = "РЕСПУБЛИКА ГВИНЕЯ-БИСАУ",
                            Name = "ГВИНЕЯ-БИСАУ"
                        },
                        new
                        {
                            Id = 53L,
                            Alpha2 = "DE",
                            Alpha3 = "DEU",
                            Code = 276L,
                            Fullname = "ФЕДЕРАТИВНАЯ РЕСПУБЛИКА ГЕРМАНИЯ",
                            Name = "ГЕРМАНИЯ"
                        },
                        new
                        {
                            Id = 54L,
                            Alpha2 = "GG",
                            Alpha3 = "GGY",
                            Code = 831L,
                            Fullname = "ГЕРНСИ",
                            Name = "ГЕРНСИ"
                        },
                        new
                        {
                            Id = 55L,
                            Alpha2 = "GI",
                            Alpha3 = "GIB",
                            Code = 292L,
                            Fullname = "ГИБРАЛТАР (БРИТ.)",
                            Name = "ГИБРАЛТАР"
                        },
                        new
                        {
                            Id = 56L,
                            Alpha2 = "HN",
                            Alpha3 = "HND",
                            Code = 340L,
                            Fullname = "РЕСПУБЛИКА ГОНДУРАС",
                            Name = "ГОНДУРАС"
                        },
                        new
                        {
                            Id = 57L,
                            Alpha2 = "HK",
                            Alpha3 = "HKG",
                            Code = 344L,
                            Fullname = "СПЕЦИАЛЬНЫЙ АДМИНИСТРАТИВНЫЙ РЕГИОН КИТАЯ ГОНКОНГ",
                            Name = "ГОНКОНГ"
                        },
                        new
                        {
                            Id = 58L,
                            Alpha2 = "GD",
                            Alpha3 = "GRD",
                            Code = 308L,
                            Fullname = "ГРЕНАДА",
                            Name = "ГРЕНАДА"
                        },
                        new
                        {
                            Id = 59L,
                            Alpha2 = "GL",
                            Alpha3 = "GRL",
                            Code = 304L,
                            Fullname = "ГРЕНЛАНДИЯ",
                            Name = "ГРЕНЛАНДИЯ"
                        },
                        new
                        {
                            Id = 60L,
                            Alpha2 = "GR",
                            Alpha3 = "GRC",
                            Code = 300L,
                            Fullname = "ГРЕЧЕСКАЯ РЕСПУБЛИКА",
                            Name = "ГРЕЦИЯ"
                        },
                        new
                        {
                            Id = 61L,
                            Alpha2 = "GE",
                            Alpha3 = "GEO",
                            Code = 268L,
                            Fullname = "РЕСПУБЛИКА ГРУЗИЯ",
                            Name = "ГРУЗИЯ"
                        },
                        new
                        {
                            Id = 62L,
                            Alpha2 = "GU",
                            Alpha3 = "GUM",
                            Code = 316L,
                            Fullname = "ГУАМ (США)",
                            Name = "ГУАМ"
                        },
                        new
                        {
                            Id = 63L,
                            Alpha2 = "DK",
                            Alpha3 = "DNK",
                            Code = 208L,
                            Fullname = "КОРОЛЕВСТВО ДАНИЯ",
                            Name = "ДАНИЯ"
                        },
                        new
                        {
                            Id = 64L,
                            Alpha2 = "JE",
                            Alpha3 = "JEY",
                            Code = 832L,
                            Fullname = "ДЖЕРСИ",
                            Name = "ДЖЕРСИ"
                        },
                        new
                        {
                            Id = 65L,
                            Alpha2 = "DJ",
                            Alpha3 = "DJI",
                            Code = 262L,
                            Fullname = "РЕСПУБЛИКА ДЖИБУТИ",
                            Name = "ДЖИБУТИ"
                        },
                        new
                        {
                            Id = 66L,
                            Alpha2 = "DM",
                            Alpha3 = "DMA",
                            Code = 212L,
                            Fullname = "СОДРУЖЕСТВО ДОМИНИКИ",
                            Name = "ДОМИНИКА"
                        },
                        new
                        {
                            Id = 67L,
                            Alpha2 = "DO",
                            Alpha3 = "DOM",
                            Code = 214L,
                            Fullname = "ДОМИНИКАНСКАЯ РЕСПУБЛИКА",
                            Name = "ДОМИНИКАНСКАЯ РЕСПУБЛИКА"
                        },
                        new
                        {
                            Id = 68L,
                            Alpha2 = "EG",
                            Alpha3 = "EGY",
                            Code = 818L,
                            Fullname = "АРАБСКАЯ РЕСПУБЛИКА ЕГИПЕТ (АРЕ)",
                            Name = "ЕГИПЕТ"
                        },
                        new
                        {
                            Id = 69L,
                            Alpha2 = "ZM",
                            Alpha3 = "ZMB",
                            Code = 894L,
                            Fullname = "РЕСПУБЛИКА ЗАМБИЯ",
                            Name = "ЗАМБИЯ"
                        },
                        new
                        {
                            Id = 70L,
                            Alpha2 = "EH",
                            Alpha3 = "ESH",
                            Code = 732L,
                            Fullname = "ЗАПАДНАЯ САХАРА",
                            Name = "ЗАПАДНАЯ САХАРА"
                        },
                        new
                        {
                            Id = 71L,
                            Alpha2 = "ZW",
                            Alpha3 = "ZWE",
                            Code = 716L,
                            Fullname = "РЕСПУБЛИКА ЗИМБАБВЕ",
                            Name = "ЗИМБАБВЕ"
                        },
                        new
                        {
                            Id = 72L,
                            Alpha2 = "IL",
                            Alpha3 = "ISR",
                            Code = 376L,
                            Fullname = "ГОСУДАРСТВО ИЗРАИЛЬ",
                            Name = "ИЗРАИЛЬ"
                        },
                        new
                        {
                            Id = 73L,
                            Alpha2 = "IN",
                            Alpha3 = "IND",
                            Code = 356L,
                            Fullname = "РЕСПУБЛИКА ИНДИЯ",
                            Name = "ИНДИЯ"
                        },
                        new
                        {
                            Id = 74L,
                            Alpha2 = "ID",
                            Alpha3 = "IDN",
                            Code = 360L,
                            Fullname = "РЕСПУБЛИКА ИНДОНЕЗИЯ",
                            Name = "ИНДОНЕЗИЯ"
                        },
                        new
                        {
                            Id = 75L,
                            Alpha2 = "JO",
                            Alpha3 = "JOR",
                            Code = 400L,
                            Fullname = "ИОРДАНСКОЕ ХАШИМИТСКОЕ КОРОЛЕВСТВО",
                            Name = "ИОРДАНИЯ"
                        },
                        new
                        {
                            Id = 76L,
                            Alpha2 = "IQ",
                            Alpha3 = "IRQ",
                            Code = 368L,
                            Fullname = "РЕСПУБЛИКА ИРАК",
                            Name = "ИРАК"
                        },
                        new
                        {
                            Id = 77L,
                            Alpha2 = "IR",
                            Alpha3 = "IRN",
                            Code = 364L,
                            Fullname = "ИСЛАМСКАЯ РЕСПУБЛИКА ИРАН",
                            Name = "ИРАН"
                        },
                        new
                        {
                            Id = 78L,
                            Alpha2 = "IE",
                            Alpha3 = "IRL",
                            Code = 372L,
                            Fullname = "ИРЛАНДИЯ",
                            Name = "ИРЛАНДИЯ"
                        },
                        new
                        {
                            Id = 79L,
                            Alpha2 = "IS",
                            Alpha3 = "ISL",
                            Code = 352L,
                            Fullname = "РЕСПУБЛИКА ИСЛАНДИЯ",
                            Name = "ИСЛАНДИЯ"
                        },
                        new
                        {
                            Id = 80L,
                            Alpha2 = "ES",
                            Alpha3 = "ESP",
                            Code = 724L,
                            Fullname = "КОРОЛЕВСТВО ИСПАНИЯ",
                            Name = "ИСПАНИЯ"
                        },
                        new
                        {
                            Id = 81L,
                            Alpha2 = "IT",
                            Alpha3 = "ITA",
                            Code = 380L,
                            Fullname = "ИТАЛЬЯНСКАЯ РЕСПУБЛИКА",
                            Name = "ИТАЛИЯ"
                        },
                        new
                        {
                            Id = 82L,
                            Alpha2 = "YE",
                            Alpha3 = "YEM",
                            Code = 887L,
                            Fullname = "ЙЕМЕНСКАЯ РЕСПУБЛИКА",
                            Name = "ЙЕМЕН"
                        },
                        new
                        {
                            Id = 83L,
                            Alpha2 = "CV",
                            Alpha3 = "CPV",
                            Code = 132L,
                            Fullname = "РЕСПУБЛИКА КАБО-ВЕРДЕ",
                            Name = "КАБО-ВЕРДЕ"
                        },
                        new
                        {
                            Id = 84L,
                            Alpha2 = "KZ",
                            Alpha3 = "KAZ",
                            Code = 398L,
                            Fullname = "РЕСПУБЛИКА КАЗАХСТАН",
                            Name = "КАЗАХСТАН"
                        },
                        new
                        {
                            Id = 85L,
                            Alpha2 = "KY",
                            Alpha3 = "CYM",
                            Code = 136L,
                            Fullname = "ОСТРОВА КАЙМАН",
                            Name = "КАЙМАН"
                        },
                        new
                        {
                            Id = 86L,
                            Alpha2 = "KH",
                            Alpha3 = "KHM",
                            Code = 116L,
                            Fullname = "КОРОЛЕВСТВО КАМБОДЖА",
                            Name = "КАМБОДЖА"
                        },
                        new
                        {
                            Id = 87L,
                            Alpha2 = "CM",
                            Alpha3 = "CMR",
                            Code = 120L,
                            Fullname = "РЕСПУБЛИКА КАМЕРУН",
                            Name = "КАМЕРУН"
                        },
                        new
                        {
                            Id = 88L,
                            Alpha2 = "CA",
                            Alpha3 = "CAN",
                            Code = 124L,
                            Fullname = "КАНАДА",
                            Name = "КАНАДА"
                        },
                        new
                        {
                            Id = 89L,
                            Alpha2 = "QA",
                            Alpha3 = "QAT",
                            Code = 634L,
                            Fullname = "ГОСУДАРСТВО КАТАР",
                            Name = "КАТАР"
                        },
                        new
                        {
                            Id = 91L,
                            Alpha2 = "KE",
                            Alpha3 = "KEN",
                            Code = 404L,
                            Fullname = "РЕСПУБЛИКА КЕНИЯ",
                            Name = "КЕНИЯ"
                        },
                        new
                        {
                            Id = 92L,
                            Alpha2 = "CY",
                            Alpha3 = "CYP",
                            Code = 196L,
                            Fullname = "РЕСПУБЛИКА КИПР",
                            Name = "КИПР"
                        },
                        new
                        {
                            Id = 93L,
                            Alpha2 = "KI",
                            Alpha3 = "KIR",
                            Code = 296L,
                            Fullname = "РЕСПУБЛИКА КИРИБАТИ",
                            Name = "КИРИБАТИ"
                        },
                        new
                        {
                            Id = 94L,
                            Alpha2 = "CN",
                            Alpha3 = "CHN",
                            Code = 156L,
                            Fullname = "КИТАЙСКАЯ НАРОДНАЯ РЕСПУБЛИКА (КНР)",
                            Name = "КИТАЙ"
                        },
                        new
                        {
                            Id = 95L,
                            Alpha2 = "CC",
                            Alpha3 = "CCK",
                            Code = 166L,
                            Fullname = "КОКОСОВЫЕ (КИЛИНГ) ОСТРОВА",
                            Name = "КОКОСОВЫЕ О-ВА"
                        },
                        new
                        {
                            Id = 96L,
                            Alpha2 = "CO",
                            Alpha3 = "COL",
                            Code = 170L,
                            Fullname = "РЕСПУБЛИКА КОЛУМБИЯ",
                            Name = "КОЛУМБИЯ"
                        },
                        new
                        {
                            Id = 97L,
                            Alpha2 = "KM",
                            Alpha3 = "COM",
                            Code = 174L,
                            Fullname = "СОЮЗ КОМОРЫ",
                            Name = "КОМОРЫ"
                        },
                        new
                        {
                            Id = 98L,
                            Alpha2 = "CG",
                            Alpha3 = "COG",
                            Code = 178L,
                            Fullname = "РЕСПУБЛИКА КОНГО",
                            Name = "КОНГО"
                        },
                        new
                        {
                            Id = 99L,
                            Alpha2 = "CD",
                            Alpha3 = "COD",
                            Code = 180L,
                            Fullname = "ДЕМОКРАТИЧЕСКАЯ РЕСПУБЛИКА КОНГО",
                            Name = "КОНГО"
                        },
                        new
                        {
                            Id = 100L,
                            Alpha2 = "KR",
                            Alpha3 = "KOR",
                            Code = 410L,
                            Fullname = "РЕСПУБЛИКА КОРЕЯ",
                            Name = "КОРЕЯ"
                        },
                        new
                        {
                            Id = 101L,
                            Alpha2 = "KP",
                            Alpha3 = "PRK",
                            Code = 408L,
                            Fullname = "КОРЕЙСКАЯ НАРОДНО-ДЕМОКРАТИЧЕСКАЯ РЕСПУБЛИКА",
                            Name = "КОРЕЯ (КНДР)"
                        },
                        new
                        {
                            Id = 102L,
                            Alpha2 = "CR",
                            Alpha3 = "CRI",
                            Code = 188L,
                            Fullname = "РЕСПУБЛИКА КОСТА-РИКА",
                            Name = "КОСТА-РИКА"
                        },
                        new
                        {
                            Id = 103L,
                            Alpha2 = "CI",
                            Alpha3 = "CIV",
                            Code = 384L,
                            Fullname = "РЕСПУБЛИКА КОТ Д'ИВУАР'",
                            Name = "КОТ Д'ИВУАР'"
                        },
                        new
                        {
                            Id = 104L,
                            Alpha2 = "CU",
                            Alpha3 = "CUB",
                            Code = 192L,
                            Fullname = "РЕСПУБЛИКА КУБА",
                            Name = "КУБА"
                        },
                        new
                        {
                            Id = 105L,
                            Alpha2 = "KW",
                            Alpha3 = "KWT",
                            Code = 414L,
                            Fullname = "ГОСУДАРСТВО КУВЕЙТ",
                            Name = "КУВЕЙТ"
                        },
                        new
                        {
                            Id = 106L,
                            Alpha2 = "KG",
                            Alpha3 = "KGZ",
                            Code = 417L,
                            Fullname = "РЕСПУБЛИКА КЫРГЫЗСТАН",
                            Name = "КЫРГЫЗСТАН"
                        },
                        new
                        {
                            Id = 107L,
                            Alpha2 = "LA",
                            Alpha3 = "LAO",
                            Code = 418L,
                            Fullname = "ЛАОССКАЯ НАРОДНО-ДЕМОКРАТИЧЕСКАЯ РЕСПУБЛИКА",
                            Name = "ЛАОС"
                        },
                        new
                        {
                            Id = 108L,
                            Alpha2 = "LV",
                            Alpha3 = "LVA",
                            Code = 428L,
                            Fullname = "ЛАТВИЙСКАЯ РЕСПУБЛИКА",
                            Name = "ЛАТВИЯ"
                        },
                        new
                        {
                            Id = 109L,
                            Alpha2 = "LS",
                            Alpha3 = "LSO",
                            Code = 426L,
                            Fullname = "КОРОЛЕВСТВО ЛЕСОТО",
                            Name = "ЛЕСОТО"
                        },
                        new
                        {
                            Id = 110L,
                            Alpha2 = "LR",
                            Alpha3 = "LBR",
                            Code = 430L,
                            Fullname = "РЕСПУБЛИКА ЛИБЕРИЯ",
                            Name = "ЛИБЕРИЯ"
                        },
                        new
                        {
                            Id = 111L,
                            Alpha2 = "LB",
                            Alpha3 = "LBN",
                            Code = 422L,
                            Fullname = "ЛИВАНСКАЯ РЕСПУБЛИКА",
                            Name = "ЛИВАН"
                        },
                        new
                        {
                            Id = 112L,
                            Alpha2 = "LY",
                            Alpha3 = "LBY",
                            Code = 434L,
                            Fullname = "СОЦИАЛИСТИЧЕСКАЯ НАРОДНАЯ ЛИВИЙСКАЯ АРАБСКАЯ ДЖАМАХИРИЯ",
                            Name = "ЛИВИЯ"
                        },
                        new
                        {
                            Id = 113L,
                            Alpha2 = "LT",
                            Alpha3 = "LTU",
                            Code = 440L,
                            Fullname = "ЛИТОВСКАЯ РЕСПУБЛИКА",
                            Name = "ЛИТВА"
                        },
                        new
                        {
                            Id = 114L,
                            Alpha2 = "LI",
                            Alpha3 = "LIE",
                            Code = 438L,
                            Fullname = "КНЯЖЕСТВО ЛИХТЕНШТЕЙН",
                            Name = "ЛИХТЕНШТЕЙН"
                        },
                        new
                        {
                            Id = 115L,
                            Alpha2 = "LU",
                            Alpha3 = "LUX",
                            Code = 442L,
                            Fullname = "ВЕЛИКОЕ ГЕРЦОГСТВО ЛЮКСЕМБУРГ",
                            Name = "ЛЮКСЕМБУРГ"
                        },
                        new
                        {
                            Id = 116L,
                            Alpha2 = "MU",
                            Alpha3 = "MUS",
                            Code = 480L,
                            Fullname = "РЕСПУБЛИКА МАВРИКИЙ",
                            Name = "МАВРИКИЙ"
                        },
                        new
                        {
                            Id = 117L,
                            Alpha2 = "MR",
                            Alpha3 = "MRT",
                            Code = 478L,
                            Fullname = "ИСЛАМСКАЯ РЕСПУБЛИКА МАВРИТАНИЯ",
                            Name = "МАВРИТАНИЯ"
                        },
                        new
                        {
                            Id = 118L,
                            Alpha2 = "MG",
                            Alpha3 = "MDG",
                            Code = 450L,
                            Fullname = "ДЕМОКРАТИЧЕСКАЯ РЕСПУБЛИКА МАДАГАСКАР",
                            Name = "МАДАГАСКАР"
                        },
                        new
                        {
                            Id = 119L,
                            Alpha2 = "YT",
                            Alpha3 = "MYT",
                            Code = 175L,
                            Fullname = "МАЙОТТА",
                            Name = "МАЙОТТА"
                        },
                        new
                        {
                            Id = 120L,
                            Alpha2 = "MO",
                            Alpha3 = "MAC",
                            Code = 446L,
                            Fullname = "СПЕЦИАЛЬНЫЙ АДМИНИСТРАТИВНЫЙ РЕГИОН КИТАЯ МАКАО",
                            Name = "МАКАО"
                        },
                        new
                        {
                            Id = 121L,
                            Alpha2 = "MK",
                            Alpha3 = "MKD",
                            Code = 807L,
                            Fullname = "РЕСПУБЛИКА МАКЕДОНИЯ",
                            Name = "МАКЕДОНИЯ"
                        },
                        new
                        {
                            Id = 122L,
                            Alpha2 = "MW",
                            Alpha3 = "MWI",
                            Code = 454L,
                            Fullname = "РЕСПУБЛИКА МАЛАВИ",
                            Name = "МАЛАВИ"
                        },
                        new
                        {
                            Id = 123L,
                            Alpha2 = "MY",
                            Alpha3 = "MYS",
                            Code = 458L,
                            Fullname = "МАЛАЙЗИЯ",
                            Name = "МАЛАЙЗИЯ"
                        },
                        new
                        {
                            Id = 124L,
                            Alpha2 = "ML",
                            Alpha3 = "MLI",
                            Code = 466L,
                            Fullname = "РЕСПУБЛИКА МАЛИ",
                            Name = "МАЛИ"
                        },
                        new
                        {
                            Id = 125L,
                            Alpha2 = "UM",
                            Alpha3 = "UMI",
                            Code = 581L,
                            Fullname = "МАЛЫЕ ТИХООКЕАНСКИЕ ОТДАЛЕННЫЕ ОСТРОВА (США)",
                            Name = "МАЛЫЕ ТИХООК. ОСТРОВА (США)"
                        },
                        new
                        {
                            Id = 126L,
                            Alpha2 = "MV",
                            Alpha3 = "MDV",
                            Code = 462L,
                            Fullname = "МАЛЬДИВСКАЯ РЕСПУБЛИКА",
                            Name = "МАЛЬДИВЫ"
                        },
                        new
                        {
                            Id = 127L,
                            Alpha2 = "MT",
                            Alpha3 = "MLT",
                            Code = 470L,
                            Fullname = "РЕСПУБЛИКА МАЛЬТА",
                            Name = "МАЛЬТА"
                        },
                        new
                        {
                            Id = 128L,
                            Alpha2 = "MP",
                            Alpha3 = "MNP",
                            Code = 580L,
                            Fullname = "СОДРУЖЕСТВО СЕВЕРНЫХ МАРИАНСКИХ ОСТРОВОВ",
                            Name = "МАРИАНСКИЕ ОСТРОВА"
                        },
                        new
                        {
                            Id = 129L,
                            Alpha2 = "MA",
                            Alpha3 = "MAR",
                            Code = 504L,
                            Fullname = "КОРОЛЕВСТВО МАРОККО",
                            Name = "МАРОККО"
                        },
                        new
                        {
                            Id = 130L,
                            Alpha2 = "MQ",
                            Alpha3 = "MTQ",
                            Code = 474L,
                            Fullname = "МАРТИНИКА (ФР.)",
                            Name = "МАРТИНИКА"
                        },
                        new
                        {
                            Id = 131L,
                            Alpha2 = "MH",
                            Alpha3 = "MHL",
                            Code = 584L,
                            Fullname = "РЕСПУБЛИКА МАРШАЛЛОВЫ ОСТРОВА",
                            Name = "МАРШАЛЛОВЫ ОСТРОВА"
                        },
                        new
                        {
                            Id = 132L,
                            Alpha2 = "MX",
                            Alpha3 = "MEX",
                            Code = 484L,
                            Fullname = "МЕКСИКАНСКИЕ СОЕДИНЕННЫЕ ШТАТЫ",
                            Name = "МЕКСИКА"
                        },
                        new
                        {
                            Id = 133L,
                            Alpha2 = "FM",
                            Alpha3 = "FSM",
                            Code = 583L,
                            Fullname = "ФЕДЕРАТИВНЫЕ ШТАТЫ МИКРОНЕЗИИ",
                            Name = "МИКРОНЕЗИЯ"
                        },
                        new
                        {
                            Id = 134L,
                            Alpha2 = "MZ",
                            Alpha3 = "MOZ",
                            Code = 508L,
                            Fullname = "РЕСПУБЛИКА МОЗАМБИК",
                            Name = "МОЗАМБИК"
                        },
                        new
                        {
                            Id = 135L,
                            Alpha2 = "MD",
                            Alpha3 = "MDA",
                            Code = 498L,
                            Fullname = "РЕСПУБЛИКА МОЛДОВА",
                            Name = "МОЛДОВА"
                        },
                        new
                        {
                            Id = 136L,
                            Alpha2 = "MC",
                            Alpha3 = "MCO",
                            Code = 492L,
                            Fullname = "КНЯЖЕСТВО МОНАКО",
                            Name = "МОНАКО"
                        },
                        new
                        {
                            Id = 137L,
                            Alpha2 = "MN",
                            Alpha3 = "MHG",
                            Code = 496L,
                            Fullname = "МОНГОЛИЯ",
                            Name = "МОНГОЛИЯ"
                        },
                        new
                        {
                            Id = 138L,
                            Alpha2 = "MS",
                            Alpha3 = "MSR",
                            Code = 500L,
                            Fullname = "МОНТСЕРРАТ (БРИТ.)",
                            Name = "МОНТСЕРРАТ"
                        },
                        new
                        {
                            Id = 139L,
                            Alpha2 = "MM",
                            Alpha3 = "MMR",
                            Code = 104L,
                            Fullname = "СОЮЗ МЬЯНМА",
                            Name = "МЬЯНМА"
                        },
                        new
                        {
                            Id = 140L,
                            Alpha2 = "NA",
                            Alpha3 = "NAM",
                            Code = 516L,
                            Fullname = "РЕСПУБЛИКА НАМИБИЯ",
                            Name = "НАМИБИЯ"
                        },
                        new
                        {
                            Id = 141L,
                            Alpha2 = "NR",
                            Alpha3 = "NRU",
                            Code = 520L,
                            Fullname = "РЕСПУБЛИКА НАУРУ",
                            Name = "НАУРУ"
                        },
                        new
                        {
                            Id = 142L,
                            Alpha2 = "NP",
                            Alpha3 = "NPL",
                            Code = 524L,
                            Fullname = "КОРОЛЕВСТВО НЕПАЛ",
                            Name = "НЕПАЛ"
                        },
                        new
                        {
                            Id = 143L,
                            Alpha2 = "NE",
                            Alpha3 = "NER",
                            Code = 562L,
                            Fullname = "РЕСПУБЛИКА НИГЕР",
                            Name = "НИГЕР"
                        },
                        new
                        {
                            Id = 144L,
                            Alpha2 = "NG",
                            Alpha3 = "NGA",
                            Code = 566L,
                            Fullname = "ФЕДЕРАТИВНАЯ РЕСПУБЛИКА НИГЕРИЯ",
                            Name = "НИГЕРИЯ"
                        },
                        new
                        {
                            Id = 145L,
                            Alpha2 = "AN",
                            Alpha3 = "ANT",
                            Code = 530L,
                            Fullname = "НИДЕРЛАНДСКИЕ АНТИЛЫ",
                            Name = "НИДЕРЛАНДСКИЕ АНТИЛЫ"
                        },
                        new
                        {
                            Id = 146L,
                            Alpha2 = "NL",
                            Alpha3 = "NLD",
                            Code = 528L,
                            Fullname = "КОРОЛЕВСТВО НИДЕРЛАНДЫ",
                            Name = "НИДЕРЛАНДЫ"
                        },
                        new
                        {
                            Id = 147L,
                            Alpha2 = "NI",
                            Alpha3 = "NIC",
                            Code = 558L,
                            Fullname = "РЕСПУБЛИКА НИКАРАГУА",
                            Name = "НИКАРАГУА"
                        },
                        new
                        {
                            Id = 148L,
                            Alpha2 = "NU",
                            Alpha3 = "NIU",
                            Code = 570L,
                            Fullname = "РЕСПУБЛИКА НИУЭ",
                            Name = "НИУЭ"
                        },
                        new
                        {
                            Id = 149L,
                            Alpha2 = "NZ",
                            Alpha3 = "NZL",
                            Code = 554L,
                            Fullname = "НОВАЯ ЗЕЛАНДИЯ",
                            Name = "НОВАЯ ЗЕЛАНДИЯ"
                        },
                        new
                        {
                            Id = 150L,
                            Alpha2 = "NC",
                            Alpha3 = "NCL",
                            Code = 540L,
                            Fullname = "НОВАЯ КАЛЕДОНИЯ",
                            Name = "НОВАЯ КАЛЕДОНИЯ"
                        },
                        new
                        {
                            Id = 151L,
                            Alpha2 = "NO",
                            Alpha3 = "NOR",
                            Code = 578L,
                            Fullname = "КОРОЛЕВСТВО НОРВЕГИЯ",
                            Name = "НОРВЕГИЯ"
                        },
                        new
                        {
                            Id = 152L,
                            Alpha2 = "NF",
                            Alpha3 = "NFK",
                            Code = 574L,
                            Fullname = "ОСТРОВ НОРФОЛК",
                            Name = "НОРФОЛК"
                        },
                        new
                        {
                            Id = 153L,
                            Alpha2 = "AE",
                            Alpha3 = "ARE",
                            Code = 784L,
                            Fullname = "ОБЪЕДИНЕННЫЕ АРАБСКИЕ ЭМИРАТЫ",
                            Name = "ОБЪЕД. АРАБСКИЕ ЭМИРАТЫ"
                        },
                        new
                        {
                            Id = 154L,
                            Alpha2 = "IM",
                            Alpha3 = "IMY",
                            Code = 833L,
                            Fullname = "ОСТРОВ МЭН",
                            Name = "О-В МЭН"
                        },
                        new
                        {
                            Id = 155L,
                            Alpha2 = "CX",
                            Alpha3 = "CXR",
                            Code = 162L,
                            Fullname = "ОСТРОВ РОЖДЕСТВА (АВСТРАЛ.)",
                            Name = "О-В РОЖДЕСТВА"
                        },
                        new
                        {
                            Id = 156L,
                            Alpha2 = "СК",
                            Alpha3 = "COK",
                            Code = 184L,
                            Fullname = "ОСТРОВА КУКА (Н. ЗЕЛ.)",
                            Name = "О-ВА КУКА"
                        },
                        new
                        {
                            Id = 157L,
                            Alpha2 = "OM",
                            Alpha3 = "OMN",
                            Code = 512L,
                            Fullname = "СУЛТАНАТ ОМАН",
                            Name = "ОМАН"
                        },
                        new
                        {
                            Id = 158L,
                            Alpha2 = "PK",
                            Alpha3 = "PAK",
                            Code = 586L,
                            Fullname = "ИСЛАМСКАЯ РЕСПУБЛИКА ПАКИСТАН",
                            Name = "ПАКИСТАН"
                        },
                        new
                        {
                            Id = 159L,
                            Alpha2 = "PW",
                            Alpha3 = "PLW",
                            Code = 585L,
                            Fullname = "РЕСПУБЛИКА ПАЛАУ",
                            Name = "ПАЛАУ"
                        },
                        new
                        {
                            Id = 160L,
                            Alpha2 = "PS",
                            Alpha3 = "PSE",
                            Code = 275L,
                            Fullname = "ОККУПИРОВАННАЯ ПАЛЕСТИНСКАЯ ТЕРРИТОРИЯ",
                            Name = "ПАЛЕСТИНСКАЯ ТЕРРИТОРИЯ, ОККУПИРОВАННАЯ"
                        },
                        new
                        {
                            Id = 161L,
                            Alpha2 = "PA",
                            Alpha3 = "PAN",
                            Code = 591L,
                            Fullname = "РЕСПУБЛИКА ПАНАМА",
                            Name = "ПАНАМА"
                        },
                        new
                        {
                            Id = 162L,
                            Alpha2 = "PG",
                            Alpha3 = "PNG",
                            Code = 598L,
                            Fullname = "ПАПУА - НОВАЯ ГВИНЕЯ",
                            Name = "ПАПУА - НОВАЯ ГВИНЕЯ"
                        },
                        new
                        {
                            Id = 163L,
                            Alpha2 = "PY",
                            Alpha3 = "PRY",
                            Code = 600L,
                            Fullname = "РЕСПУБЛИКА ПАРАГВАЙ",
                            Name = "ПАРАГВАЙ"
                        },
                        new
                        {
                            Id = 164L,
                            Alpha2 = "PE",
                            Alpha3 = "PER",
                            Code = 604L,
                            Fullname = "РЕСПУБЛИКА ПЕРУ",
                            Name = "ПЕРУ"
                        },
                        new
                        {
                            Id = 165L,
                            Alpha2 = "PN",
                            Alpha3 = "PCN",
                            Code = 612L,
                            Fullname = "ПИТКЭРН (БРИТ.)",
                            Name = "ПИТКЭРН"
                        },
                        new
                        {
                            Id = 166L,
                            Alpha2 = "PL",
                            Alpha3 = "POL",
                            Code = 616L,
                            Fullname = "РЕСПУБЛИКА ПОЛЬША",
                            Name = "ПОЛЬША"
                        },
                        new
                        {
                            Id = 167L,
                            Alpha2 = "PT",
                            Alpha3 = "PRT",
                            Code = 620L,
                            Fullname = "ПОРТУГАЛЬСКАЯ РЕСПУБЛИКА",
                            Name = "ПОРТУГАЛИЯ"
                        },
                        new
                        {
                            Id = 168L,
                            Alpha2 = "PR",
                            Alpha3 = "PRI",
                            Code = 630L,
                            Fullname = "ПУЭРТО-РИКО",
                            Name = "ПУЭРТО-РИКО"
                        },
                        new
                        {
                            Id = 169L,
                            Alpha2 = "RE",
                            Alpha3 = "REU",
                            Code = 638L,
                            Fullname = "РЕЮНЬОН",
                            Name = "РЕЮНЬОН"
                        },
                        new
                        {
                            Id = 170L,
                            Alpha2 = "RU",
                            Alpha3 = "RUS",
                            Code = 643L,
                            Fullname = "РОССИЙСКАЯ ФЕДЕРАЦИЯ",
                            Name = "РОССИЯ"
                        },
                        new
                        {
                            Id = 171L,
                            Alpha2 = "RW",
                            Alpha3 = "RWA",
                            Code = 646L,
                            Fullname = "РУАНДИЙСКАЯ РЕСПУБЛИКА",
                            Name = "РУАНДА"
                        },
                        new
                        {
                            Id = 172L,
                            Alpha2 = "RO",
                            Alpha3 = "ROM",
                            Code = 642L,
                            Fullname = "РУМЫНИЯ",
                            Name = "РУМЫНИЯ"
                        },
                        new
                        {
                            Id = 173L,
                            Alpha2 = "WS",
                            Alpha3 = "WSM",
                            Code = 882L,
                            Fullname = "НЕЗАВИСИМОЕ ГОСУДАРСТВО САМОА",
                            Name = "САМОА"
                        },
                        new
                        {
                            Id = 174L,
                            Alpha2 = "ST",
                            Alpha3 = "STR",
                            Code = 678L,
                            Fullname = "ДЕМОКРАТИЧЕСКАЯ РЕСПУБЛИКА САН-ТОМЕ И ПРИНСИПИ",
                            Name = "САН-ТОМЕ И ПРИНСИПИ"
                        },
                        new
                        {
                            Id = 175L,
                            Alpha2 = "SM",
                            Alpha3 = "SMR",
                            Code = 674L,
                            Fullname = "РЕСПУБЛИКА САН-МАРИНО",
                            Name = "САН-МАРИНО"
                        },
                        new
                        {
                            Id = 176L,
                            Alpha2 = "SA",
                            Alpha3 = "SAU",
                            Code = 682L,
                            Fullname = "КОРОЛЕВСТВО САУДОВСКАЯ АРАВИЯ",
                            Name = "САУДОВСКАЯ АРАВИЯ"
                        },
                        new
                        {
                            Id = 177L,
                            Alpha2 = "SZ",
                            Alpha3 = "SWZ",
                            Code = 748L,
                            Fullname = "КОРОЛЕВСТВО СВАЗИЛЕНД",
                            Name = "СВАЗИЛЕНД"
                        },
                        new
                        {
                            Id = 178L,
                            Alpha2 = "SH",
                            Alpha3 = "SHN",
                            Code = 654L,
                            Fullname = "ОСТРОВ СВЯТОЙ ЕЛЕНЫ (БРИТ.)",
                            Name = "СВЯТАЯ ЕЛЕНА"
                        },
                        new
                        {
                            Id = 179L,
                            Alpha2 = "SC",
                            Alpha3 = "SYC",
                            Code = 690L,
                            Fullname = "РЕСПУБЛИКА СЕЙШЕЛЫ",
                            Name = "СЕЙШЕЛЫ"
                        },
                        new
                        {
                            Id = 180L,
                            Alpha2 = "PM",
                            Alpha3 = "SPM",
                            Code = 666L,
                            Fullname = "СЕН-ПЬЕР И МИКЕЛОН (ФР.)",
                            Name = "СЕН-ПЬЕР И МИКЕЛОН"
                        },
                        new
                        {
                            Id = 181L,
                            Alpha2 = "SN",
                            Alpha3 = "SEN",
                            Code = 686L,
                            Fullname = "РЕСПУБЛИКА СЕНЕГАЛ",
                            Name = "СЕНЕГАЛ"
                        },
                        new
                        {
                            Id = 182L,
                            Alpha2 = "VC",
                            Alpha3 = "VCT",
                            Code = 670L,
                            Fullname = "СЕНТ-ВИНСЕНТ И ГРЕНАДИНЫ",
                            Name = "СЕНТ-ВИНСЕНТ И ГРЕНАДИНЫ"
                        },
                        new
                        {
                            Id = 183L,
                            Alpha2 = "KN",
                            Alpha3 = "KNA",
                            Code = 659L,
                            Fullname = "ФЕДЕРАЦИЯ СЕНТ-КИТС (СЕНТ-КРИСТОФЕР) И НЕВИС",
                            Name = "СЕНТ-КИТС И НЕВИС"
                        },
                        new
                        {
                            Id = 184L,
                            Alpha2 = "LC",
                            Alpha3 = "LCA",
                            Code = 662L,
                            Fullname = "СЕНТ-ЛЮСИЯ",
                            Name = "СЕНТ-ЛЮСИЯ"
                        },
                        new
                        {
                            Id = 185L,
                            Alpha2 = "RS",
                            Alpha3 = "SRB",
                            Code = 688L,
                            Fullname = "РЕСПУБЛИКА СЕРБИЯ",
                            Name = "СЕРБИЯ"
                        },
                        new
                        {
                            Id = 186L,
                            Alpha2 = "SG",
                            Alpha3 = "SGP",
                            Code = 702L,
                            Fullname = "РЕСПУБЛИКА СИНГАПУР",
                            Name = "СИНГАПУР"
                        },
                        new
                        {
                            Id = 187L,
                            Alpha2 = "SY",
                            Alpha3 = "SYR",
                            Code = 760L,
                            Fullname = "СИРИЙСКАЯ АРАБСКАЯ РЕСПУБЛИКА",
                            Name = "СИРИЯ"
                        },
                        new
                        {
                            Id = 188L,
                            Alpha2 = "SK",
                            Alpha3 = "SVK",
                            Code = 703L,
                            Fullname = "СЛОВАЦКАЯ РЕСПУБЛИКА",
                            Name = "СЛОВАКИЯ"
                        },
                        new
                        {
                            Id = 189L,
                            Alpha2 = "SI",
                            Alpha3 = "SVN",
                            Code = 705L,
                            Fullname = "РЕСПУБЛИКА СЛОВЕНИЯ",
                            Name = "СЛОВЕНИЯ"
                        },
                        new
                        {
                            Id = 190L,
                            Alpha2 = "GB",
                            Alpha3 = "GBR",
                            Code = 826L,
                            Fullname = "СОЕДИНЕННОЕ КОРОЛЕВСТВО ВЕЛИКОБРИТАНИИ И СЕВЕРНОЙ ИРЛАНДИИ",
                            Name = "СОЕДИНЕННОЕ КОРОЛЕВСТВО"
                        },
                        new
                        {
                            Id = 191L,
                            Alpha2 = "SB",
                            Alpha3 = "SLB",
                            Code = 90L,
                            Fullname = "СОЛОМОНОВЫ ОСТРОВА",
                            Name = "СОЛОМОНОВЫ О-ВА"
                        },
                        new
                        {
                            Id = 192L,
                            Alpha2 = "SO",
                            Alpha3 = "SOM",
                            Code = 706L,
                            Fullname = "СОМАЛИЙСКАЯ РЕСПУБЛИКА",
                            Name = "СОМАЛИ"
                        },
                        new
                        {
                            Id = 193L,
                            Alpha2 = "SD",
                            Alpha3 = "SDN",
                            Code = 736L,
                            Fullname = "РЕСПУБЛИКА СУДАН",
                            Name = "СУДАН"
                        },
                        new
                        {
                            Id = 194L,
                            Alpha2 = "SR",
                            Alpha3 = "SUR",
                            Code = 740L,
                            Fullname = "РЕСПУБЛИКА СУРИНАМ",
                            Name = "СУРИНАМ"
                        },
                        new
                        {
                            Id = 195L,
                            Alpha2 = "US",
                            Alpha3 = "USA",
                            Code = 840L,
                            Fullname = "СОЕДИНЕННЫЕ ШТАТЫ АМЕРИКИ",
                            Name = "США"
                        },
                        new
                        {
                            Id = 196L,
                            Alpha2 = "SL",
                            Alpha3 = "SLE",
                            Code = 694L,
                            Fullname = "РЕСПУБЛИКА СЬЕРРА-ЛЕОНЕ",
                            Name = "СЬЕРРА-ЛЕОНЕ"
                        },
                        new
                        {
                            Id = 197L,
                            Alpha2 = "TJ",
                            Alpha3 = "TJK",
                            Code = 762L,
                            Fullname = "РЕСПУБЛИКА ТАДЖИКИСТАН",
                            Name = "ТАДЖИКИСТАН"
                        },
                        new
                        {
                            Id = 198L,
                            Alpha2 = "TH",
                            Alpha3 = "THA",
                            Code = 764L,
                            Fullname = "КОРОЛЕВСТВО ТАИЛАНД",
                            Name = "ТАИЛАНД"
                        },
                        new
                        {
                            Id = 199L,
                            Alpha2 = "TW",
                            Alpha3 = "TWN",
                            Code = 158L,
                            Fullname = "ТАЙВАНЬ (В СОСТАВЕ КИТАЯ)",
                            Name = "ТАЙВАНЬ"
                        },
                        new
                        {
                            Id = 200L,
                            Alpha2 = "TZ",
                            Alpha3 = "TZA",
                            Code = 834L,
                            Fullname = "ОБЪЕДИНЕННАЯ РЕСПУБЛИКА ТАНЗАНИЯ (ОРТ)",
                            Name = "ТАНЗАНИЯ"
                        },
                        new
                        {
                            Id = 201L,
                            Alpha2 = "TC",
                            Alpha3 = "TCA",
                            Code = 796L,
                            Fullname = "ОСТРОВА ТЕРКС И КАЙКОС (БРИТ.)",
                            Name = "ТЕРКС И КАЙКОС"
                        },
                        new
                        {
                            Id = 202L,
                            Alpha2 = "TP",
                            Alpha3 = "TMP",
                            Code = 626L,
                            Fullname = "ДЕМОКРАТИЧЕСКАЯ РЕСПУБЛИКА ТИМОР-ЛЕСТЕ",
                            Name = "ТИМОР-ЛЕСТЕ"
                        },
                        new
                        {
                            Id = 203L,
                            Alpha2 = "TG",
                            Alpha3 = "TGO",
                            Code = 768L,
                            Fullname = "ТОГОЛЕЗСКАЯ РЕСПУБЛИКА",
                            Name = "ТОГО"
                        },
                        new
                        {
                            Id = 204L,
                            Alpha2 = "TK",
                            Alpha3 = "TKL",
                            Code = 772L,
                            Fullname = "ТОКЕЛАУ (ЮНИОН) (Н. ЗЕЛ.)",
                            Name = "ТОКЕЛАУ"
                        },
                        new
                        {
                            Id = 205L,
                            Alpha2 = "TO",
                            Alpha3 = "TON",
                            Code = 776L,
                            Fullname = "КОРОЛЕВСТВО ТОНГА",
                            Name = "ТОНГА"
                        },
                        new
                        {
                            Id = 206L,
                            Alpha2 = "TT",
                            Alpha3 = "TTO",
                            Code = 780L,
                            Fullname = "РЕСПУБЛИКА ТРИНИДАД И ТОБАГО",
                            Name = "ТРИНИДАД И ТОБАГО"
                        },
                        new
                        {
                            Id = 207L,
                            Alpha2 = "TV",
                            Alpha3 = "TUV",
                            Code = 798L,
                            Fullname = "ТУВАЛУ",
                            Name = "ТУВАЛУ"
                        },
                        new
                        {
                            Id = 208L,
                            Alpha2 = "TN",
                            Alpha3 = "TUN",
                            Code = 788L,
                            Fullname = "ТУНИССКАЯ РЕСПУБЛИКА",
                            Name = "ТУНИС"
                        },
                        new
                        {
                            Id = 209L,
                            Alpha2 = "TM",
                            Alpha3 = "TKM",
                            Code = 795L,
                            Fullname = "ТУРКМЕНИСТАН",
                            Name = "ТУРКМЕНИЯ"
                        },
                        new
                        {
                            Id = 210L,
                            Alpha2 = "TR",
                            Alpha3 = "TUR",
                            Code = 792L,
                            Fullname = "ТУРЕЦКАЯ РЕСПУБЛИКА",
                            Name = "ТУРЦИЯ"
                        },
                        new
                        {
                            Id = 211L,
                            Alpha2 = "UG",
                            Alpha3 = "UGA",
                            Code = 800L,
                            Fullname = "РЕСПУБЛИКА УГАНДА",
                            Name = "УГАНДА"
                        },
                        new
                        {
                            Id = 212L,
                            Alpha2 = "UZ",
                            Alpha3 = "UZB",
                            Code = 860L,
                            Fullname = "РЕСПУБЛИКА УЗБЕКИСТАН",
                            Name = "УЗБЕКИСТАН"
                        },
                        new
                        {
                            Id = 213L,
                            Alpha2 = "UA",
                            Alpha3 = "UKR",
                            Code = 804L,
                            Fullname = "УКРАИНА",
                            Name = "УКРАИНА"
                        },
                        new
                        {
                            Id = 214L,
                            Alpha2 = "WF",
                            Alpha3 = "WLF",
                            Code = 876L,
                            Fullname = "ОСТРОВА УОЛЛИС И ФУТУНА",
                            Name = "УОЛЛИС И ФУТУНА"
                        },
                        new
                        {
                            Id = 215L,
                            Alpha2 = "UY",
                            Alpha3 = "URY",
                            Code = 858L,
                            Fullname = "ВОСТОЧНАЯ РЕСПУБЛИКА УРУГВАЙ",
                            Name = "УРУГВАЙ"
                        },
                        new
                        {
                            Id = 216L,
                            Alpha2 = "FO",
                            Alpha3 = "FRO",
                            Code = 234L,
                            Fullname = "ФАРЕРСКИЕ ОСТРОВА (В СОСТАВЕ ДАНИИ)",
                            Name = "ФАРЕРСКИЕ О-ВА"
                        },
                        new
                        {
                            Id = 217L,
                            Alpha2 = "FJ",
                            Alpha3 = "FJI",
                            Code = 242L,
                            Fullname = "РЕСПУБЛИКА ОСТРОВОВ ФИДЖИ",
                            Name = "ФИДЖИ"
                        },
                        new
                        {
                            Id = 218L,
                            Alpha2 = "PH",
                            Alpha3 = "PHL",
                            Code = 608L,
                            Fullname = "РЕСПУБЛИКА ФИЛИППИНЫ",
                            Name = "ФИЛИППИНЫ"
                        },
                        new
                        {
                            Id = 219L,
                            Alpha2 = "FI",
                            Alpha3 = "FIN",
                            Code = 246L,
                            Fullname = "ФИНЛЯНДСКАЯ РЕСПУБЛИКА",
                            Name = "ФИНЛЯНДИЯ"
                        },
                        new
                        {
                            Id = 220L,
                            Alpha2 = "FK",
                            Alpha3 = "FLK",
                            Code = 238L,
                            Fullname = "ФОЛКЛЕНДСКИЕ ОСТРОВА (МАЛЬВИНСКИЕ)",
                            Name = "ФОЛКЛЕНДСКИЕ О-ВА"
                        },
                        new
                        {
                            Id = 221L,
                            Alpha2 = "TF",
                            Alpha3 = "ATF",
                            Code = 260L,
                            Fullname = "ФРАНЦУЗСКИЕ ЮЖНЫЕ ТЕРРИТОРИИ (ФР.)",
                            Name = "ФР. ЮЖНЫЕ ТЕРРИТОРИИ"
                        },
                        new
                        {
                            Id = 222L,
                            Alpha2 = "FR",
                            Alpha3 = "FRA",
                            Code = 250L,
                            Fullname = "ФРАНЦУЗСКАЯ РЕСПУБЛИКА",
                            Name = "ФРАНЦИЯ"
                        },
                        new
                        {
                            Id = 223L,
                            Alpha2 = "PF",
                            Alpha3 = "PYF",
                            Code = 258L,
                            Fullname = "ФРАНЦУЗСКАЯ ПОЛИНЕЗИЯ (ФР.)",
                            Name = "ФРАНЦУЗСКАЯ ПОЛИНЕЗИЯ"
                        },
                        new
                        {
                            Id = 224L,
                            Alpha2 = "HM",
                            Alpha3 = "HMD",
                            Code = 334L,
                            Fullname = "ОСТРОВ ХЕРД И ОСТРОВА МАКДОНАЛЬД",
                            Name = "ХЕРД И МАКДОНАЛЬД"
                        },
                        new
                        {
                            Id = 225L,
                            Alpha2 = "HR",
                            Alpha3 = "HRV",
                            Code = 191L,
                            Fullname = "РЕСПУБЛИКА ХОРВАТИЯ",
                            Name = "ХОРВАТИЯ"
                        },
                        new
                        {
                            Id = 226L,
                            Alpha2 = "CF",
                            Alpha3 = "CAF",
                            Code = 140L,
                            Fullname = "ЦЕНТРАЛЬНО-АФРИКАНСКАЯ РЕСПУБЛИКА (ЦАР)",
                            Name = "ЦЕНТР. - АФР. РЕСПУБЛИКА"
                        },
                        new
                        {
                            Id = 227L,
                            Alpha2 = "TD",
                            Alpha3 = "TCD",
                            Code = 148L,
                            Fullname = "РЕСПУБЛИКА ЧАД",
                            Name = "ЧАД"
                        },
                        new
                        {
                            Id = 228L,
                            Alpha2 = "ME",
                            Alpha3 = "MNE",
                            Code = 499L,
                            Fullname = "РЕСПУБЛИКА ЧЕРНОГОРИЯ",
                            Name = "ЧЕРНОГОРИЯ"
                        },
                        new
                        {
                            Id = 229L,
                            Alpha2 = "CZ",
                            Alpha3 = "CZE",
                            Code = 203L,
                            Fullname = "ЧЕШСКАЯ РЕСПУБЛИКА",
                            Name = "ЧЕХИЯ"
                        },
                        new
                        {
                            Id = 230L,
                            Alpha2 = "CL",
                            Alpha3 = "CHL",
                            Code = 152L,
                            Fullname = "РЕСПУБЛИКА ЧИЛИ",
                            Name = "ЧИЛИ"
                        },
                        new
                        {
                            Id = 231L,
                            Alpha2 = "CH",
                            Alpha3 = "CHE",
                            Code = 756L,
                            Fullname = "ШВЕЙЦАРСКАЯ КОНФЕДЕРАЦИЯ",
                            Name = "ШВЕЙЦАРИЯ"
                        },
                        new
                        {
                            Id = 232L,
                            Alpha2 = "SE",
                            Alpha3 = "SWE",
                            Code = 752L,
                            Fullname = "КОРОЛЕВСТВО ШВЕЦИЯ",
                            Name = "ШВЕЦИЯ"
                        },
                        new
                        {
                            Id = 233L,
                            Alpha2 = "SJ",
                            Alpha3 = "SJM",
                            Code = 744L,
                            Fullname = "ШПИЦБЕРГЕН И ЯН-МАЙЕН (НОРВ.)",
                            Name = "ШПИЦБЕРГЕН И ЯН-МАЙЕН"
                        },
                        new
                        {
                            Id = 234L,
                            Alpha2 = "LK",
                            Alpha3 = "LKA",
                            Code = 144L,
                            Fullname = "ДЕМОКРАТИЧЕСКАЯ СОЦИАЛИСТИЧЕСКАЯ РЕСПУБЛИКА ШРИ-ЛАНКА",
                            Name = "ШРИ-ЛАНКА"
                        },
                        new
                        {
                            Id = 235L,
                            Alpha2 = "EC",
                            Alpha3 = "ECU",
                            Code = 218L,
                            Fullname = "РЕСПУБЛИКА ЭКВАДОР",
                            Name = "ЭКВАДОР"
                        },
                        new
                        {
                            Id = 236L,
                            Alpha2 = "GQ",
                            Alpha3 = "GNQ",
                            Code = 226L,
                            Fullname = "РЕСПУБЛИКА ЭКВАТОРИАЛЬНАЯ ГВИНЕЯ",
                            Name = "ЭКВАТОРИАЛЬНАЯ ГВИНЕЯ"
                        },
                        new
                        {
                            Id = 237L,
                            Alpha2 = "AX",
                            Alpha3 = "ALA",
                            Code = 248L,
                            Fullname = "ЭЛАНДСКИЕ ОСТРОВА",
                            Name = "ЭЛАНДСКИЕ ОСТРОВА"
                        },
                        new
                        {
                            Id = 238L,
                            Alpha2 = "SV",
                            Alpha3 = "SLV",
                            Code = 222L,
                            Fullname = "РЕСПУБЛИКА ЭЛ-САЛЬВАДОР",
                            Name = "ЭЛЬ-САЛЬВАДОР"
                        },
                        new
                        {
                            Id = 239L,
                            Alpha2 = "ER",
                            Alpha3 = "ERI",
                            Code = 232L,
                            Fullname = "ЭРИТРЕЯ",
                            Name = "ЭРИТРЕЯ"
                        },
                        new
                        {
                            Id = 240L,
                            Alpha2 = "EE",
                            Alpha3 = "EST",
                            Code = 233L,
                            Fullname = "ЭСТОНСКАЯ РЕСПУБЛИКА",
                            Name = "ЭСТОНИЯ"
                        },
                        new
                        {
                            Id = 241L,
                            Alpha2 = "ET",
                            Alpha3 = "ETH",
                            Code = 231L,
                            Fullname = "ФЕДЕРАТИВНАЯ ДЕМОКРАТИЧЕСКАЯ  РЕСПУБЛИКА ЭФИОПИЯ",
                            Name = "ЭФИОПИЯ"
                        },
                        new
                        {
                            Id = 242L,
                            Alpha2 = "ZA",
                            Alpha3 = "ZAF",
                            Code = 710L,
                            Fullname = "ЮЖНО-АФРИКАНСКАЯ РЕСПУБЛИКА",
                            Name = "ЮЖНАЯ АФРИКА"
                        },
                        new
                        {
                            Id = 243L,
                            Alpha2 = "GS",
                            Alpha3 = "SGS",
                            Code = 239L,
                            Fullname = "ЮЖНАЯ ДЖОРДЖИЯ И ЮЖНЫЕ САНДВИЧЕВЫ ОСТРОВА",
                            Name = "ЮЖНАЯ ДЖОРДЖИЯ И ЮЖНЫЕ САНДВИЧЕВЫ ОСТРОВА"
                        },
                        new
                        {
                            Id = 244L,
                            Alpha2 = "JM",
                            Alpha3 = "JAM",
                            Code = 388L,
                            Fullname = "ЯМАЙКА",
                            Name = "ЯМАЙКА"
                        },
                        new
                        {
                            Id = 245L,
                            Alpha2 = "JP",
                            Alpha3 = "JPN",
                            Code = 392L,
                            Fullname = "ЯПОНИЯ",
                            Name = "ЯПОНИЯ"
                        });
                });

            modelBuilder.Entity("TeamHost.Entities.Game", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Description")
                        .HasColumnType("text");

                    b.Property<long>("DeveloperId")
                        .HasColumnType("bigint");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<decimal>("Price")
                        .HasColumnType("numeric");

                    b.Property<float>("Rating")
                        .HasColumnType("real");

                    b.Property<DateTime>("ReleaseDate")
                        .HasColumnType("date");

                    b.Property<string>("ShortDescription")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("StaticFilePath")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("DeveloperId");

                    b.ToTable("Games");

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Description = "Build an empire to stand the test of time.",
                            DeveloperId = 1L,
                            Name = "Civilization VI",
                            Price = 999m,
                            Rating = 4.5f,
                            ReleaseDate = new DateTime(2016, 10, 21, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            ShortDescription = "Build an empire to stand the test of time.",
                            StaticFilePath = "~/img/games/civilization-vi.jpeg"
                        },
                        new
                        {
                            Id = 2L,
                            Description = "Hunt down the Child of Prophecy in a vast open world.",
                            DeveloperId = 2L,
                            Name = "The Witcher 3: Wild Hunt",
                            Price = 1499m,
                            Rating = 4.8f,
                            ReleaseDate = new DateTime(2015, 5, 19, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            ShortDescription = "Hunt down the Child of Prophecy in a vast open world.",
                            StaticFilePath = "~/img/games/the-witcher-3.jpeg"
                        },
                        new
                        {
                            Id = 3L,
                            Description = "An online multiplayer social deduction game.",
                            DeveloperId = 3L,
                            Name = "Among Us",
                            Price = 499m,
                            Rating = 4.7f,
                            ReleaseDate = new DateTime(2018, 6, 15, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            ShortDescription = "Online multiplayer social deduction game.",
                            StaticFilePath = "~/img/games/among-us.jpeg"
                        },
                        new
                        {
                            Id = 4L,
                            Description = "An action-adventure game played from either a first-person or third-person perspective.",
                            DeveloperId = 4L,
                            Name = "Grand Theft Auto V",
                            Price = 1999m,
                            Rating = 4.9f,
                            ReleaseDate = new DateTime(2013, 9, 17, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            ShortDescription = "Action-adventure game.",
                            StaticFilePath = "~/img/games/gta5.jpeg"
                        },
                        new
                        {
                            Id = 5L,
                            Description = "A survival horror game developed and published by Capcom.",
                            DeveloperId = 5L,
                            Name = "Resident Evil Village",
                            Price = 2799m,
                            Rating = 4.6f,
                            ReleaseDate = new DateTime(2021, 5, 7, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            ShortDescription = "Survival horror game.",
                            StaticFilePath = "~/img/games/re-village.jpeg"
                        },
                        new
                        {
                            Id = 6L,
                            Description = "A farming simulation game.",
                            DeveloperId = 6L,
                            Name = "Stardew Valley",
                            Price = 699m,
                            Rating = 4.7f,
                            ReleaseDate = new DateTime(2016, 2, 26, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            ShortDescription = "Farming simulation game.",
                            StaticFilePath = "~/img/games/stardew-valley.jpeg"
                        },
                        new
                        {
                            Id = 7L,
                            Description = "An action role-playing game set in a third-person perspective.",
                            DeveloperId = 7L,
                            Name = "Dark Souls III",
                            Price = 2399m,
                            Rating = 4.9f,
                            ReleaseDate = new DateTime(2016, 4, 12, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            ShortDescription = "Action role-playing game.",
                            StaticFilePath = "~/img/games/dark-souls-3.jpeg"
                        },
                        new
                        {
                            Id = 8L,
                            Description = "An asymmetrical multiplayer horror game.",
                            DeveloperId = 8L,
                            Name = "Dead by Daylight",
                            Price = 1499m,
                            Rating = 4.6f,
                            ReleaseDate = new DateTime(2016, 6, 14, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            ShortDescription = "Asymmetrical multiplayer horror game.",
                            StaticFilePath = "~/img/games/dead-by-daylight.jpeg"
                        },
                        new
                        {
                            Id = 9L,
                            Description = "An action-adventure game set in a post-apocalyptic world.",
                            DeveloperId = 9L,
                            Name = "The Last of Us Part II",
                            Price = 2999m,
                            Rating = 4.7f,
                            ReleaseDate = new DateTime(2020, 6, 19, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            ShortDescription = "Action-adventure game.",
                            StaticFilePath = "~/img/games/the-last-of-us-2.jpeg"
                        },
                        new
                        {
                            Id = 10L,
                            Description = "A sandbox video game developed and published by Mojang Studios.",
                            DeveloperId = 10L,
                            Name = "Minecraft",
                            Price = 1999m,
                            Rating = 4.6f,
                            ReleaseDate = new DateTime(2011, 11, 18, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            ShortDescription = "Sandbox video game.",
                            StaticFilePath = "~/img/games/minecraft.jpeg"
                        });
                });

            modelBuilder.Entity("TeamHost.Entities.GameCategory", b =>
                {
                    b.Property<long>("GameId")
                        .HasColumnType("bigint");

                    b.Property<long>("CategoryId")
                        .HasColumnType("bigint");

                    b.HasKey("GameId", "CategoryId");

                    b.HasIndex("CategoryId");

                    b.ToTable("GameCategories");

                    b.HasData(
                        new
                        {
                            GameId = 1L,
                            CategoryId = 1L
                        },
                        new
                        {
                            GameId = 2L,
                            CategoryId = 2L
                        },
                        new
                        {
                            GameId = 3L,
                            CategoryId = 1L
                        },
                        new
                        {
                            GameId = 4L,
                            CategoryId = 2L
                        },
                        new
                        {
                            GameId = 5L,
                            CategoryId = 3L
                        },
                        new
                        {
                            GameId = 6L,
                            CategoryId = 4L
                        },
                        new
                        {
                            GameId = 7L,
                            CategoryId = 2L
                        },
                        new
                        {
                            GameId = 8L,
                            CategoryId = 3L
                        },
                        new
                        {
                            GameId = 9L,
                            CategoryId = 2L
                        },
                        new
                        {
                            GameId = 10L,
                            CategoryId = 2L
                        });
                });

            modelBuilder.Entity("TeamHost.Entities.GamePlatform", b =>
                {
                    b.Property<long>("GameId")
                        .HasColumnType("bigint");

                    b.Property<long>("PlatformId")
                        .HasColumnType("bigint");

                    b.Property<long?>("GamePlatformGameId")
                        .HasColumnType("bigint");

                    b.Property<long?>("GamePlatformPlatformId")
                        .HasColumnType("bigint");

                    b.HasKey("GameId", "PlatformId");

                    b.HasIndex("PlatformId");

                    b.HasIndex("GamePlatformGameId", "GamePlatformPlatformId");

                    b.ToTable("GamePlatforms");

                    b.HasData(
                        new
                        {
                            GameId = 1L,
                            PlatformId = 1L
                        },
                        new
                        {
                            GameId = 2L,
                            PlatformId = 1L
                        },
                        new
                        {
                            GameId = 3L,
                            PlatformId = 1L
                        },
                        new
                        {
                            GameId = 4L,
                            PlatformId = 1L
                        },
                        new
                        {
                            GameId = 5L,
                            PlatformId = 1L
                        },
                        new
                        {
                            GameId = 6L,
                            PlatformId = 1L
                        },
                        new
                        {
                            GameId = 7L,
                            PlatformId = 1L
                        },
                        new
                        {
                            GameId = 8L,
                            PlatformId = 1L
                        },
                        new
                        {
                            GameId = 9L,
                            PlatformId = 2L
                        },
                        new
                        {
                            GameId = 10L,
                            PlatformId = 1L
                        });
                });

            modelBuilder.Entity("TeamHost.Entities.Platform", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<long?>("ImageId")
                        .HasColumnType("bigint");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("ImageId");

                    b.ToTable("Platforms");

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Code = "windows",
                            Name = "Windows"
                        },
                        new
                        {
                            Id = 2L,
                            Code = "apple",
                            Name = "Apple"
                        });
                });

            modelBuilder.Entity("TeamHost.Entities.StaticFile", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Extension")
                        .HasColumnType("text");

                    b.Property<long?>("GameId")
                        .HasColumnType("bigint");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Path")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int?>("Size")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("GameId");

                    b.ToTable("StaticFiles");

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Name = "civilization-vi.jpeg",
                            Path = "~/img/games/civilization-vi.jpeg",
                            Size = 10
                        },
                        new
                        {
                            Id = 2L,
                            Name = "the-witcher-3.jpeg",
                            Path = "~/img/games/the-witcher-3.jpeg",
                            Size = 11
                        },
                        new
                        {
                            Id = 3L,
                            Name = "among-us.jpeg",
                            Path = "~/img/games/among-us.jpeg",
                            Size = 8
                        },
                        new
                        {
                            Id = 4L,
                            Name = "gta5.jpeg",
                            Path = "~/img/games/gta5.jpeg",
                            Size = 16
                        },
                        new
                        {
                            Id = 5L,
                            Name = "re-village.jpeg",
                            Path = "~/img/games/re-village.jpeg",
                            Size = 7
                        },
                        new
                        {
                            Id = 6L,
                            Name = "stardew-valley.jpeg",
                            Path = "~/img/games/stardew-valley.jpeg",
                            Size = 16
                        },
                        new
                        {
                            Id = 7L,
                            Name = "dark-souls-3.jpeg",
                            Path = "~/img/games/dark-souls-3.jpeg",
                            Size = 10
                        },
                        new
                        {
                            Id = 8L,
                            Name = "dead-by-daylight.jpeg",
                            Path = "~/img/games/dead-by-daylight.jpeg",
                            Size = 11
                        },
                        new
                        {
                            Id = 9L,
                            Name = "the-last-of-us.jpeg",
                            Path = "~/img/games/the-last-of-us.jpeg",
                            Size = 9
                        },
                        new
                        {
                            Id = 10L,
                            Name = "minecraft.jpeg",
                            Path = "~/img/games/minecraft.jpeg",
                            Size = 17
                        });
                });

            modelBuilder.Entity("GamePlatform", b =>
                {
                    b.HasOne("TeamHost.Entities.Game", null)
                        .WithMany()
                        .HasForeignKey("GamesId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("TeamHost.Entities.Platform", null)
                        .WithMany()
                        .HasForeignKey("PlatformsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("TeamHost.Entities.Company", b =>
                {
                    b.HasOne("TeamHost.Entities.Country", "Country")
                        .WithMany()
                        .HasForeignKey("CountryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Country");
                });

            modelBuilder.Entity("TeamHost.Entities.Game", b =>
                {
                    b.HasOne("TeamHost.Entities.Company", "Developer")
                        .WithMany()
                        .HasForeignKey("DeveloperId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Developer");
                });

            modelBuilder.Entity("TeamHost.Entities.GameCategory", b =>
                {
                    b.HasOne("TeamHost.Entities.Category", "Category")
                        .WithMany("GameCategories")
                        .HasForeignKey("CategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("TeamHost.Entities.Game", "Game")
                        .WithMany("GameCategories")
                        .HasForeignKey("GameId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Category");

                    b.Navigation("Game");
                });

            modelBuilder.Entity("TeamHost.Entities.GamePlatform", b =>
                {
                    b.HasOne("TeamHost.Entities.Game", "Game")
                        .WithMany("GamePlatforms")
                        .HasForeignKey("GameId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("TeamHost.Entities.Platform", "Platform")
                        .WithMany()
                        .HasForeignKey("PlatformId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("TeamHost.Entities.GamePlatform", null)
                        .WithMany("GamePlatforms")
                        .HasForeignKey("GamePlatformGameId", "GamePlatformPlatformId");

                    b.Navigation("Game");

                    b.Navigation("Platform");
                });

            modelBuilder.Entity("TeamHost.Entities.Platform", b =>
                {
                    b.HasOne("TeamHost.Entities.StaticFile", "Image")
                        .WithMany()
                        .HasForeignKey("ImageId");

                    b.Navigation("Image");
                });

            modelBuilder.Entity("TeamHost.Entities.StaticFile", b =>
                {
                    b.HasOne("TeamHost.Entities.Game", null)
                        .WithMany("Images")
                        .HasForeignKey("GameId");
                });

            modelBuilder.Entity("TeamHost.Entities.Category", b =>
                {
                    b.Navigation("GameCategories");
                });

            modelBuilder.Entity("TeamHost.Entities.Game", b =>
                {
                    b.Navigation("GameCategories");

                    b.Navigation("GamePlatforms");

                    b.Navigation("Images");
                });

            modelBuilder.Entity("TeamHost.Entities.GamePlatform", b =>
                {
                    b.Navigation("GamePlatforms");
                });
#pragma warning restore 612, 618
        }
    }
}
